    ...
    def _load_tuls(self) -> List[TulInfo]:
        ...
        tuls_data = [
            ...
            TulInfo(
                ...
                history="Назван в память о патриоте Ан Джун-Гуне (1879-1910), который убил японского генерал-губернатора Кореи Ито Хиробуми. 32 движения символизируют возраст, в котором его казнили в тюрьме Люйшунь.",
...
class TaekwondoTulBot:
    ...
    def __init__(self):
        ...
        # Маппинг для кнопок поясов (ключ меню поясов -> полное название)
        self.belt_key_to_full = {
            "10_gup": "10 гып (белый пояс)",
            "9_gup": "9 гып (белый с желтой полоской)",
            "8_gup": "8 гып (желтый пояс)",
            "7_gup": "7 гып (желтый с зеленой полоской)",
            "6_gup": "6 гып (зеленый пояс)",
            "5_gup": "5 гып (зеленый с синей полоска)",
            "4_gup": "4 гып (синий пояс)",
            "3_gup": "3 гып (синий с красной полоской)",
            "2_gup": "2 гып (красный пояс)",
            "1_gup": "1 гып (красный с черной полоской)",
            "1_dan": "1 дан (черный пояс)",
            "2_dan": "2 дан (черный пояс)",
            "3_dan": "3 дан (черный пояс)",
            "4_dan": "4 дан (черный пояс)",
            "5_dan": "5 дан (черный пояс)",
            "6_dan": "6 дан (черный пояс)"
        }
        ...
        for belt_key, belt_info in self.knowledge.belts.items():
            self.belt_key_to_full[belt_key] = f"{belt_info['name']} - {belt_info['color']}"
    ...
    def get_tuls_menu(self) -> InlineKeyboardMarkup:
        ...
        for belt, tuls in belt_groups.items():
            ...
            belt_short_name = None
            for short, full in self.belt_short_to_full.items():
                if full == belt:
                    belt_short_name = short
                    break

            if belt_short_name:
                # ИСПРАВЛЕНО: используем 'belt_' префикс
                keyboard.append([InlineKeyboardButton(f"📖 {belt_short}", callback_data=f'belt_{belt_short_name}')])
            else:
                # Если не нашли в маппинге, используем начало строки
                belt_key = belt.split("(")[0].strip() if "(" in belt else belt
                # ИСПРАВЛЕНО: используем 'belt_' префикс
                keyboard.append([InlineKeyboardButton(f"📖 {belt_short}", callback_data=f'belt_{belt_key}')])
    ...
    def get_belts_menu(self) -> InlineKeyboardMarkup:
        ...
        for i in range(0, len(belts_order), 2):
            ...
            if i < len(belts_order):
                ...
                # Убедитесь, что используется 'belt_info_' префикс
    ...
    def _find_belt_by_query(self, query: str) -> Optional[Tuple[str, Dict]]:
        """Найти пояс по запросу"""
        query_lower = query.lower()

        # Словарь для сопоставления различных вариантов написания
        belt_variants = {
            # 10 гып
            ...
            # Даны
            "1 дан": ["1 дан", "1-дан", "первый дан", "черный пояс 1 дан", "1st dan", "1st"],
            "2 дан": ["2 дан", "2-дан", "второй дан", "черный пояс 2 дан", "2nd dan", "2nd"],
            "3 дан": ["3 дан", "3-дан", "третий дан", "черный пояс 3 дан", "3rd dan", "3rd"],
            "4 дан": ["4 дан", "4-дан", "четвертый дан", "черный пояс 4 дан", "4th dan"],
            "5 дан": ["5 дан", "5-дан", "пятый дан", "черный пояс 5 дан", "5th dan"],
            "6 дан": ["6 дан", "6-дан", "шестой дан", "черный пояс 6 дан", "6th dan"],
            "черный пояс": ["черный пояс", "черный", "black belt", "black", "дан", "черный"]
        }

        # Проверяем все варианты написания
        for belt_display, variants in belt_variants.items():
            for variant in variants:
                if variant in query_lower:
                    # Нашли совпадение, теперь находим соответствующий пояс
                    for belt_key, belt_info in self.knowledge.belts.items():
                        # Проверяем имя пояса и цвет
                        if (belt_info["name"].lower() in query_lower or
                                belt_info["color"].lower() in query_lower or
                                belt_display.lower() in belt_info["name"].lower() or
                                belt_display.lower() in belt_info["color"].lower()):
                            return belt_key, belt_info

        # Прямой поиск по базе поясов
        for belt_key, belt_info in self.knowledge.belts.items():
            # Проверяем имя пояса
            if query_lower in belt_info["name"].lower():
                return belt_key, belt_info

            # Проверяем цвет пояса
            if query_lower in belt_info["color"].lower():
                return belt_key, belt_info

            # Проверяем ключевые слова в описании
            if query_lower in belt_info["description"].lower():
                return belt_key, belt_info

            # Проверяем требования
            if "requirements" in belt_info and query_lower in belt_info["requirements"].lower():
                return belt_key, belt_info
...
Выберите раздел в меню или напишите название туля или пояса!
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            # Сначала пробуем найти тул по точному названию
            tul = self.knowledge.get_tul_by_name(query)
            if tul:
                await self._send_tul_info(update, tul)
                return

            # Если не нашли по точному названию, ищем по ключевым словам в тулях
            ...
            # Пробуем найти пояс по разным вариантам
            belt_match = self._find_belt_by_query(query)
...
{chr(10).join(tuls_info) if tuls_info else '• Нет тулей для этого уровня'}
        ...
        keyboard = []
        if tuls_info:
            # Находим соответствующий ключ для callback_data в меню тулей
            for short_name, full_belt in self.belt_short_to_full.items():
                if belt_info["name"] == short_name or belt_info["name"] in full_belt:
                    # ИСПРАВЛЕНО: используем 'belt_' вместо 'info_'
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if data == 'back_to_menu':
                ...
                # ИСПРАВЛЕНИЕ: обрабатываем старый формат 'info_'
                belt_key = data.replace('info_', '')
                # Пробуем показать информацию о поясе
                belt_info = self.knowledge.get_belt_info(belt_key)
                if belt_info:
                    await self._show_belt_info_from_menu(query, belt_key)
                else:
                    # Если не нашли по ключу, пробуем показать тули
                    await self._show_tuls_for_belt(query, belt_key.replace('_', ' '))
    ...
    async def _show_tuls_for_belt(self, query, belt_query: str):
        ...
        # Используем маппинг для поиска полного названия пояса
        full_belt = None
        if belt_query in self.belt_short_to_full:
            full_belt = self.belt_short_to_full[belt_query]

        # Если не нашли в маппинге, ищем по частичному совпадению
        if not full_belt:
            for tul in self.knowledge.tuls:
                if (belt_query.lower() in tul.belt.lower() or
                        tul.belt.lower().startswith(belt_query.lower())):
                    full_belt = tul.belt
                    break
        ...
        for i, tul in enumerate(tuls, 1):
            ...
            response += f"   • История: {tul['history'][:300]}...\n\n"
...
{chr(10).join(tuls_info) if tuls_info else '• Нет тулей для этого уровня'}
    ...
    async def _show_tul_progression(self, query):
        ...
        chunks = []
        current_chunk = "📊 <b>Временная шкала изучения тулей по поясам</b>\n\n"
        ...
        for i, item in enumerate(progression):
            ...
            if item['tuls']:
                for tul in item['tuls']:
                    item_text += f"  • {tul['name']} ({tul['movements']} дв.) - {tul['meaning'][:50]}...\n"
            else:
                item_text += "  • Нет тулей\n"

            item_text += "\n"

            # Если добавление этого элемента превысит лимит, сохраняем текущий чанк и начинаем новый
            if len(current_chunk) + len(item_text) + 50 > 4000:  # 50 символов для HTML тегов и отступов
                current_chunk += "\n<i>Продолжение следует...</i>"
                chunks.append(current_chunk)
                current_chunk = f"<b>Продолжение временной шкалы...</b>\n\n{item_text}"
            else:
                current_chunk += item_text

        # Добавляем последний чанк
        if current_chunk:
            current_chunk += "<i>Всего 24 туля, символизирующих 24 часа в сутках.</i>"
            chunks.append(current_chunk)
        ...
        # Отправляем остальные чанки как новые сообщения
    ...
    async def _show_belt_timeline(self, query):
        ...
        response += "<i>Путь длиною в жизнь! Черный пояс - это не конец, а начало настоящего пути.</i>"
...
• "5 гып" или "5-гып"
• "черный пояс" или "black belt"
• "синий с красной" (3 гып)
• "1 дан" или "первый дан"
    ...
    async def search_belt_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.args:
            ...
            if belt_match:
                ...
                # Показываем все доступные пояса
                response = "🎽 <b>Доступные пояса для поиска:</b>\n\n"
                ...
                belt_categories = {
                    "Ученические пояса (гыпы)": [
                        ...
                        "8 гып - Желтый пояс",
                        "7 гып - Желтый с зеленой полоской",
                        "6 гып - Зеленый пояс",
                        "5 гып - Зеленый с синей полоской",
                        "4 гып - Синий пояс",
                        "3 гып - Синий с красной полоской",
                        "2 гып - Красный пояс",
                        "1 гып - Красный с черной полоской"
                    ],
                    "Мастерские пояса (даны)": [
                        "1 дан - Черный пояс",
                        "2 дан - Черный пояс",
                        "3 дан - Черный пояс",
                        "4 дан - Черный пояс",
                ...
                for category, belts in belt_categories.items():
                    response += f"<b>{category}:</b>\n"
                    for belt in belts:
                        response += f"• {belt}\n"
                    response += "\n"

                response += "🔍 <b>Примеры запросов:</b>\n"
                response += "• желтый пояс\n• 5 гып\n• черный пояс\n• 1 дан\n• синий с красной"
...
def main():
    ...
    try:
        ...
        print("🔘 Исправленные кнопки поясов: ДА")
